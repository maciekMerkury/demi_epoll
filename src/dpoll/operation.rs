...
use crate::{buffer::{Buffer, Index}, shared::{Shared, ThreadBuffer}, socket::Socket, wrappers::demi};
...
impl Operation {
    pub unsafe fn from_raw(socs: &Buffer<true, Shared<Socket>>, op: c_int, fd: c_int, event: *mut epoll_event) -> Self {
    ...
    pub fn new(
        ...
        return match op {
            EPOLL_CTL_ADD => {
                ...
                Self::Add { soc, evs: evs.unwrap(), data: event.u64 }
            }
            EPOLL_CTL_DEL => Self::Del { qd: soc.borrow().soc.qd },
            EPOLL_CTL_MOD => Self::Mod { qd: soc.borrow().soc.qd, evs: evs.unwrap() },
